...
const ContactCategoryForm = (props: FormType) => {
  ...
  const onSubmit = async (data: any) => {
    try {
      setLoading(true);
      const form = {
        ...(addFlag && { id: auth?.user?.id }),
        ...data,
      };
      const response = addFlag
        ? await axios.post(`/contact-category`, form)
        : await axios.patch(`/contact-category/${currentData.id}`, form);
      setLoading(false);
  ...
  return (
    <Drawer
      ...
      sx={{ '& .Midrawer-paper': { width: { xs: 300, sm: 400 } } }}
    >
      <CustomHeader>
        <Typography variant="h6">
          {addFlag ? 'Add' : 'Edit'} Category
        </Typography>
        <IconButton
          size="small"
          onClick={handleClose}
