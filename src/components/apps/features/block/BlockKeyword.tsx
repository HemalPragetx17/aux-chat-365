...
import BlockKeywordForm from '../../../forms/features/BlockKeywordForm';
...
const BlockKeywordList = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`/block-keyword?status=ACTIVE,INACTIVE`);
  ...
  const searchData = useCallback(async () => {
    ...
    if (Boolean(search.trim())) {
      filtered = filtered.filter(
        (obj: any) =>
          obj.keyword.toLowerCase().indexOf(search.trim().toLowerCase()) > -1,
      );
    }
    if (status !== 'ALL') {
  ...
  const handleEdit = async (row: any) => {
    setLoading(true);
    const resposne = await axios.get(`/block-keyword/${row?.id}`);
  ...
  const handleDelete = async (row: any) => {
    ...
    if (willDelete) {
      try {
        setLoading(true);
        const response = await axios.delete(`/block-keyword/${row?.id}`);
...
export default BlockKeywordList;
