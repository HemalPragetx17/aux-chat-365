...
import ContactCategoryForm from '../../../forms/settings/ContactCategoryForm';
...
const ContactCategoryList = () => {
  ...
  const handleEdit = async (row: any) => {
    const resposne = await axios.get(`/contact-category/${row?.id}`);
  ...
  const handleDelete = async (row: any) => {
    ...
    if (willDelete) {
      await axios.delete(`/contact-category/${row?.id}`);
      fetchData();
    }
  };
