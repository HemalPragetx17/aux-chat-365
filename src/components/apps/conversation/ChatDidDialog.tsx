...
import { fetchChatByNumber } from '../../../store/apps/chat/ChatSlice';
...
const ChatDialog = (props: ChatDialogProp) => {
  ...
  const handleMessage = useCallback(async () => {
    ...
    const { route } = router;
    if (route.includes('conversation')) {
      dispatch(fetchChatByNumber(destination.trim(), fromNumber));
      toggle();
    } else {
      const chatNumber = JSON.stringify({ phone: destination, fromNumber });
      sessionStorage.setItem('chatNumber', chatNumber);
      router.replace(`/chats/conversation`);
    }
