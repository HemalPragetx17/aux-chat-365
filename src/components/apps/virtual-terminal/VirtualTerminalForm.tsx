...
function addPlusSignToBillingCountryCode(billingCountryCode: string) {
...
const VirtualTerminalForm = (props: FormType) => {
  const { toggle, open, profileData, success } = props;
  const dispatch = useDispatch();
  const { user } = useAuth();
  const [loading, setLoading] = useState<boolean>(false);
  const [cardType, setCardType] = useState('');
  const [invoiceDetails, setInvoiceDetails] = useState<any>(
    defaultValuesOfInvoice,
  );
  ...
  const schema = yup.object().shape({
    ...
    fullName: yup.string().required('Contact field is required'),
  ...
  useEffect(() => {
    ...
    if (profileData) {
      setValue('BillingEmail', selectedChat?.Contact?.email, {
        shouldValidate: true,
      });
      setValue(
        'fullName',
        `${selectedChat?.Contact?.firstName || ''} ${
          selectedChat?.Contact?.lastName || ''
        }`.trim(),
      );
      setValue('BillingPhoneNumber', selectedChat?.Contact?.phone, {
        shouldValidate: true,
      });
      setValue('firstName', selectedChat?.Contact?.firstName, {
        shouldValidate: true,
      });
      setValue('lastName', selectedChat?.Contact?.lastName, {
        shouldValidate: true,
      });
    }
  }, [open]);
  ...
  const continueFormSubmittion = async ({
    ...
    let payload = {
      ...
      BillingCustomerName: finalFormData?.fullName,
  ...
  const inputDate = new Date(invoiceDetails?.createdAt || new Date());
  ...
  useEffect(() => {
    ...
    setValue('BillingPostalCode', zip, { shouldValidate: true });
  }, [watchForm?.fullAddress, open]);
  ...
  return (
    ...
    >
      <div className="main" style={{ background: '#323441' }}>
            ...
            <Box gridColumn="span 6">
              <div className="left_div1" style={{ position: 'relative' }}>
                <IconButton
                  ...
                  onClick={() => {
                    toggle();
                    setInvoiceDetails(defaultValuesOfInvoice);
                  }}
                  sx={{
                    borderRadius: 1,
                    color: 'black',
                    ...
                    <Box gridColumn="span 6">
                      <ul>
                        ...
                        <li className="li_list d-flex">
                          ...
                          <p className="top-p2">
                            {cardType} {watch('CardNumber')?.slice(0, 5)}
                    ...
                    <Box gridColumn="span 12">
                      <ul>
                        ...
                        <li className="li_list customer_div">
                          ...
                          <p className="p2">{invoiceDetails?.SuggestedMode}</p>
                        </li>
                        <li className="li_list customer_div">
                          <p className="p1">Card holder:</p>
                          <p className="p2">{watchForm?.fullName}</p>
                        </li>
                        <li className="li_list customer_div">
                          <p className="p1">Authorization Code:</p>
                          <p className="p2">{invoiceDetails?.AuthCode}</p>
                        </li>
                        <li className="li_list customer_div">
                          <p className="p1">Gateway Ref: </p>
                          <p className="p2">
                            {invoiceDetails?.TransactionGateWay}
                          </p>
                        </li>
                      </ul>
                    </Box>
